...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(abp_7_2_2Consts.OidcConsts.AccessTokenKeyName);
        ...
        if (!token.IsNullOrEmpty())
        {
            ...
            {
                var refreshToken = await _storage.GetAsync(abp_7_2_2Consts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(abp_7_2_2Consts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(abp_7_2_2Consts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(abp_7_2_2Consts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(abp_7_2_2Consts.OidcConsts.RefreshTokenKeyName);
    }
}